...
if __name__ == "__main__":
    ...
    data_path_out = "B题附件/城市航线_新_1.txt"
    CITY_NUM = 8
    YUZHI = 1026
    DISCOUNT = 0.8065
    ...
    # 观察数据特点
    # print(distance_mat.info)
    # print(distance_mat.describe())

    # 记录大于阈值的城市对
    long_path = []
    for i in range(CITY_NUM):
        for j in range(i, CITY_NUM):
            if distance_mat[i][j] > YUZHI:
                long_path.append([i + 1, j + 1, distance_mat[i][j]])
    # print(long_path)

    # 所选城市对作为新航线，距离*DISCOUNT
    new_path = long_path.copy()
    for one in new_path:
        one[2] = round(one[2] * DISCOUNT)
    # print(new_path)
